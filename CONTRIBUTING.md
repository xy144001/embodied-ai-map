...
在创建 commit 或推送分支之前，维护者必须重新阅读本文件，并逐项确认：

- 修改内容只属于自己负责的目录；
- 没有未经说明的公共路由、组件、Schema、依赖或部署改动；
- 已运行 `npm run check`、`npm run validate-content` 和 `npm run build`，并记录无法通过的环境原因；
- 已检查 `git diff`、文件范围、来源链接和许可证状态；
- 未获得明确授权时，不执行远程推送、合并或权限变更。
